...
//     {
//         // First try to get embedded lyrics
//         string lyrics = file.Tag.Lyrics;

//         // If no embedded lyrics, fetch from online
//         if (string.IsNullOrEmpty(lyrics))
//         {
//             lyrics = await FetchLyrics(file.Tag.Title, file.Tag.FirstPerformer);
//         }

//         // Display lyrics in a panel
//         if (!string.IsNullOrEmpty(lyrics))
//         {
//             var lyricsPanel = new Panel(lyrics)
//             {
...
//         else
//         {
//             AnsiConsole.MarkupLine("[grey]No lyrics found.[/]");
//         }
//     }

//     static async Task<string> FetchLyrics(string? title, string? artist)
//     {
//         if (string.IsNullOrEmpty(title) || string.IsNullOrEmpty(artist) || string.IsNullOrEmpty(lyricsApiKey))
//         {
//             return null;
//         }
...
//             // Extract lyrics from the JSON
//             return json["lyrics"]?.ToString();
//         }
...
//             ApplyEqualizer(buffer, offset, samplesRead);
//             return samplesRead;
//         }

//         private void ApplyEqualizer(float[] buffer, int offset, int count)
//         {
...
//                     // Implement a simple biquad filter for each band
//                     float w0 = 2 * (float)Math.PI * frequency / sampleRate;
//                     float cosW0 = (float)Math.Cos(w0);
//                     float alpha = (float)Math.Sin(w0) / (2 * 1); // Q = 1 for simplicity

//                     float A = (float)Math.Pow(10, gain / 40); // Convert dB to linear gain
//                     float b0 = 1 + alpha * A;
//                     float b1 = -2 * cosW0;
//                     float b2 = 1 - alpha * A;
//                     float a0 = 1 + alpha / A;
...
//                     // Apply the filter (Direct Form 1)
//                     float filteredSample = (b0 * sample + b1 * 0 + b2 * 0 - a1 * 0 - a2 * 0) / a0;

//                     // Update the sample in the buffer
